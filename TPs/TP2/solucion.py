...
def skyline_incr(i, last_height, total_width, case):
    ...
    if M_incr[i][total_width] == -1: # Verifico si ya tengo solucion al subproblema
        ...
        if heights[case-1][i] > last_height: # Verifico si el skyline es creciente
            ...
            M_incr[i][total_width] = max(skyline_incr(i+1, last_height, total_width, case), skyline_incr(i+1, heights[case-1][i], total_width + widths[case-1][i], case) + widths[case-1][i])

        else: # De no ser creciente, sigo recorriendo
            M_incr[i][total_width] = skyline_incr(i+1, last_height, total_width, case)    
...
def skyline_decr(i, last_height, total_width, case): # Igual que skyline_incr pero distinta comparacion
    ...
    if M_decr[i][total_width] == -1: # Verifico si ya tengo solucion al subproblema
        ...
        if heights[case-1][i] < last_height: # Verifico si el skyline es creciente
            ...
            # Asigno el maximo entre las recursiones de excluir o incluir el elemento
            M_decr[i][total_width] = max(skyline_decr(i+1, last_height, total_width, case), skyline_decr(i+1, heights[case-1][i], total_width + widths[case-1][i], case) + widths[case-1][i])

        else: # De no ser creciente, sigo recorriendo
            M_decr[i][total_width] = skyline_decr(i+1, last_height, total_width, case)    
...
for case in range(1, T+1):
    ...
    max_incr_length = skyline_incr(0, float('-inf'), 0, case)
    max_decr_length = skyline_decr(0, float('+inf'), 0, case)
